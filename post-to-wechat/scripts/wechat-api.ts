...
import { loadWechatExtendConfig, loadCredentials } from "./wechat-extend-config.ts";
import {
...
Options:
  ...
  --min-images <n>    Require at least n body images before publishing/dry-run success
  --require-cover     Require an explicit cover path; do not use the first body image as cover fallback
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--type" && argv[i + 1]) {
      ...
      args.minImages = parsePositiveIntegerOption("--min-images", argv[++i]);
    } else if (arg === "--require-cover") {
      args.requireCover = true;
    } else if (arg === "--cite") {
...
function resolveCoverPath(rawCoverPath: string | undefined, baseDir: string): string | undefined {
  if (!rawCoverPath || isRemoteUrl(rawCoverPath) || path.isAbsolute(rawCoverPath)) {
    return rawCoverPath;
  }
  return path.resolve(baseDir, rawCoverPath);
}

function resolveNewsCoverPath(options: {
  cliCover?: string;
  frontmatter: Record<string, string>;
  baseDir: string;
}): string | undefined {
  if (options.cliCover) {
    return resolveCoverPath(options.cliCover, process.cwd());
  }

  const frontmatterCover = options.frontmatter.coverImage ||
    options.frontmatter.featureImage ||
    options.frontmatter.cover ||
    options.frontmatter.image;
  const resolvedFrontmatterCover = resolveCoverPath(frontmatterCover, options.baseDir);
  if (resolvedFrontmatterCover) {
    return resolvedFrontmatterCover;
  }

  const defaultCoverPath = path.join(options.baseDir, "imgs", "cover.png");
  return fs.existsSync(defaultCoverPath) ? defaultCoverPath : undefined;
}

function validatePublishInputs(options: {
  articleType: ArticleType;
  coverPath?: string;
  ...
  if (options.minImages > 0 && options.contentImageCount < options.minImages) {
    throw new Error(`Expected at least ${options.minImages} body images, got ${options.contentImageCount}`);
  }

  if (options.articleType === "news" && options.requireCover && !options.coverPath) {
    throw new Error("Cover image is required. Provide --cover, frontmatter coverImage, or imgs/cover.png.");
  }

  if (options.articleType === "news" && !options.coverPath && options.contentImageCount === 0) {
    throw new Error("No cover image. Provide via --cover, frontmatter.coverImage, or include an image in content.");
  }

  if (options.coverPath && !isRemoteUrl(options.coverPath) && !fs.existsSync(options.coverPath)) {
    throw new Error(`Cover image not found: ${options.coverPath}`);
  }
}
...
async function main(): Promise<void> {
  ...
  const needNewsCoverFallback = args.articleType === "news" && !coverPath && !args.requireCover;
  ...
  const result = await publishToDraft({
    ...
    needOpenComment: extConfig.need_open_comment ?? 1,
    onlyFansCanComment: extConfig.only_fans_can_comment ?? 0,
  }, accessToken);
